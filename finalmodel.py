...
class NestedUNet(nn.Module):
    def __init__(self, num_classes, input_channels=3, deep_supervision=True, **kwargs):
        ...
        self.uppp1 = VGGBlock(512,512,32)
        self.uppp2 = VGGBlock(512, 32, 32)
        self.uppp3 = VGGBlock(32, 32, 32)

        self.uppp4 = VGGBlock(256,64,32)
        self.uppp5 = VGGBlock(128, 64, 64)

        self.uppp6 = VGGBlock(128, 64, 32)

        self.upsampling = upppp(32,32)
        self.upsampling1 = upppp(64, 32)
        self.upsampling2 = upppp(128, 128)
        self.upsampling3 = upppp(256, 256)
        self.upsampling4 = upppp(16,16)
    ...
    def forward(self, input):
        ...
        if self.deep_supervision:
            ...
            x4up = self.uppp1(self.upsampling3(x4_0)) #channel reduction fronm 512 to 32
            x4up = self.upsampling4(x4up)
            x4up = self.upsampling4(x4up)
            t1 = self.upsampling4(x4up)
            # print(t1.shape, "t1")

            x3up = self.uppp4(self.upsampling2(x3_1))
            x3up = self.upsampling4(x3up)
            t2 = self.upsampling4(x3up)
            # print(t2.shape, "t2")


            x2up= self.uppp6(self.upsampling1(x2_2))
            t3 =self.upsampling4(x2up)
            # print(t3.shape, "t3")

            t4 = self.new3(self.upsampling(x1_3))
            # print(t4.shape, "t4")

            output1 = self.final1(t1)
            output2 = self.final2(t2)
            output3 = self.final3(t3)
            output4 = self.final4(t4)

            # print(output2.shape, output1.shape, output3.shape, output4.shape)
            y = (output1+ output2+ output3+ output4)/4
